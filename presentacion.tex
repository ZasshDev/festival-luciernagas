...
\author[Zassh]{Desarrollador: Zassh}
\institute[Desarrollo Independiente]{Proyecto de Desarrollo Independiente}
\date{\today}
...
\begin{frame}{Metodología(s) Utilizada(s)}
  Se implementó \textbf{Agile para Desarrollador Único (Personal Kanban)}:
  \begin{itemize}
      \item \textbf{Desarrollo Iterativo:} Entregas funcionales diarias.
      \item \textbf{Gestión de Tareas:} Flujo visual (To Do, In Progress, Done) para evitar cuellos de botella.
      \item \textbf{Sprints Diarios:} Reevaluación de prioridades cada mañana.
  \end{itemize}
  \vspace{0.3cm}
  Prácticas Complementarias:
  \begin{itemize}
      \item \textbf{AI Pair Programming:} Uso de Inteligencia Artificial como asistente para agilizar la codificación, refactorizaciones y configuración del VPS.
      \item \textbf{Control de Versiones:} Uso de Git/GitHub para respaldar iteraciones diarias.
  \end{itemize}
\end{frame}
...
% ---------------------------------------------------
\section{Bitácora}
\begin{frame}{Bitácora de Desarrollo (1 Semana de Sprint)}
  \begin{table}
      \centering
      \resizebox{\textwidth}{!}{%
      \begin{tabular}{@{}llp{6.5cm}@{}}
      \toprule
      \textbf{Día} & \textbf{Fase} & \textbf{Actividad Principal} \\ \midrule
      Miércoles (Día 1) & Planificación & Definición de requerimientos, setup del entorno y esquema de BD (Prisma). \\
      Jueves (Día 2) & Backend Core & Desarrollo de API REST, autenticación JWT y roles. \\
      Viernes (Día 3) & Frontend Base & Maquetación inicial con React y Tailwind CSS. \\
      Fin de semana & Integración & Conexión Front-Back, lógica de reservas y mapas (Leaflet). \\
      Lunes (Día 6) & UI Premium & Mejoras estéticas, SVG, animaciones (Framer Motion). \\
      Martes (Día 7) & Despliegue & Generación de PDF, tests de escáner QR y fixes de HTTPS. \\
      Miércoles (Hoy) & Producción & Puesta en producción final en Servidor VPS (Nginx/PM2). \\ \bottomrule
      \end{tabular}%
      }
  \end{table}
\end{frame}
